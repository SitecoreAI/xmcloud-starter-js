...
export const LocationSearchItem = ({
  ...
  const placeSchema = generatePlaceSchema(
    dealership.dealershipName?.jsonValue?.value?.toString() || 'Office',
  ...
  const dealershipName =
    dealership.dealershipName?.jsonValue?.value?.toString() || 'location';
  const dealershipAddress =
    dealership.dealershipAddress?.jsonValue?.value?.toString() || '';
  const uniqueId = `${dealershipName}-${dealershipAddress}`
    .replace(/[^a-zA-Z0-9-]/g, '-')
    .toLowerCase();
  ...
  return (
    <>
      ...
      <article
        ...
        onKeyDown={(e) =>
          e.key === ' ' || e.key === 'Enter' ? onSelect(dealership) : null
        }
        role="button"
        tabIndex={0}
