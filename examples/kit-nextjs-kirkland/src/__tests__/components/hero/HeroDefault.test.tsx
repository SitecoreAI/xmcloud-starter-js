...
describe('HeroDefault Component', () => {
  ...
  it('does not render an empty banner for repeated copy and an incomplete CTA', () => {
    const propsWithIncompleteBanner = {
      ...mockHeroProps,
      fields: {
    ...
    const { container } = render(
      <HeroDefault {...propsWithIncompleteBanner} />,
    );

    expect(
      container.querySelector('[data-hero-banner]'),
    ).not.toBeInTheDocument();
    expect(screen.queryByTestId('button-base')).not.toBeInTheDocument();
  });

  it('does not show an unconfigured location search in Page Builder', () => {
    ...
    render(<HeroDefault {...editingPropsWithoutSearch} />);
