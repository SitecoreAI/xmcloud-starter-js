...
describe('GlobalHeaderCentered Component', () => {
  it('renders without crashing with centered layout', () => {
    const { container } = render(
      <GlobalHeaderCentered {...mockGlobalHeaderProps} />,
    );
  ...
  it('keeps the logo field editable while showing the firm-name fallback in Page Builder', () => {
    ...
    render(<GlobalHeaderCentered {...editingPropsWithoutLogo} />);
  ...
  it('links both responsive header layouts to the localized search page', () => {
    render(
      <GlobalHeaderCentered
        {...mockGlobalHeaderProps}
        page={{ ...mockGlobalHeaderProps.page, locale: 'ja-JP' }}
      />,
    );
    ...
    searchLinks.forEach((searchLink) => {
      expect(searchLink).toHaveAttribute('href', '/ja-JP/site-search');
    });
  });
});
