...
jest.mock('@/components/ui/sheet', () => {
  const MockSheetTrigger = React.forwardRef<
    HTMLDivElement,
    MockSheetTriggerProps
  >(({ children, ...props }, ref) => {
    // Filter out asChild prop to avoid React warning (it's a Radix UI prop)
    // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
    const { asChild, ...domProps } = props as any;
    return React.createElement(
      'div',
      { ...domProps, ref, 'data-testid': 'sheet-trigger' },
      children as React.ReactNode,
    );
  });
...
describe('GlobalHeader Component', () => {
  ...
  describe('Basic rendering', () => {
    ...
    it('should render the approved fallback logo when the Sitecore logo is missing', () => {
  ...
  describe('Desktop navigation', () => {
    it('adds Who we are when the Sitecore datasource has no about link', () => {
      render(<GlobalHeader {...defaultProps} />);

      const links = screen.getAllByRole('link', { name: 'Who we are' });
    ...
    it('does not duplicate Who we are when Sitecore supplies it', () => {
      render(<GlobalHeader {...propsWithAboutLink} />);

      expect(screen.getAllByRole('link', { name: 'Who we are' })).toHaveLength(
        2,
      );
    });
  ...
  describe('Component structure', () => {
    ...
    it('should apply correct height class', () => {
      ...
      const header = container.querySelector('header');
      expect(header).toHaveClass('h-[72px]', 'lg:h-[80px]');
    });

    it('should apply responsive padding classes', () => {
      const { container } = render(<GlobalHeader {...defaultProps} />);

      const innerContainer = container.querySelector('.slb-page-shell');
      expect(innerContainer).toBeInTheDocument();
      expect(innerContainer).toHaveClass('slb-page-shell');
    });
          ...
          data: {
            item: {
              ...
              children: {} as unknown as {
                results?: Array<{ link: { jsonValue: LinkField } }>;
              },
