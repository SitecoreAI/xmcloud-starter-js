...
test('GET /.well-known/ai.txt returns 200 and valid JSON', async () => {
  ...
  expect(resp.data.includes(`AI-Endpoint: ${BASE_URL}/ai/summary.json`)).toBe(
    true,
  );
  expect(resp.data.includes(`AI-Endpoint: ${BASE_URL}/ai/faq.json`)).toBe(true);
  expect(resp.data.includes(`AI-Endpoint: ${BASE_URL}/ai/service.json`)).toBe(
    true,
  );
...
test('GET /ai/markdown/Products/Aero returns 404 and isolates legacy content', async () => {
  const resp = await axios.get(`${BASE_URL}/ai/markdown/Products/Aero`, {
    validateStatus: () => true,
    timeout: 10000,
  });
  expect(resp.status).toBe(404);
  expect(resp.headers['content-type']).toContain('text/plain');
  expect(resp.data).toBe('Page not found');
  expect(resp.data).not.toContain('Alaris');
}, 15000);
...
test('GET /sitemap-llm.xml returns 200 and valid XML', async () => {
  ...
  expect(locs).toHaveLength(expectedUrls.length);
...
test('GET /sitemap.xml returns 200 and valid XML', async () => {
  ...
  const requiredPaths = NWN_ROUTE_PATHS;
  const expectedUrls = requiredPaths.map((p) =>
