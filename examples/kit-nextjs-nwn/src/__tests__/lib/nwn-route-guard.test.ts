...
describe('NWN legacy route guard', () => {
  it('blocks inherited product and test-drive routes case-insensitively', () => {
    ...
    expect(isLegacyStarterUrl('https://www.nwnatural.com/products/Nexa')).toBe(true);
  });

  it('keeps NW Natural site routes', () => {
    expect(isLegacyStarterRoute(['account-billing'])).toBe(false);
    expect(
      isLegacyStarterUrl('https://www.nwnatural.com/safety/call-before-you-dig'),
    ).toBe(false);
  });

  it('removes only legacy URL entries from a sitemap', () => {
    const xml = `<?xml version="1.0"?><urlset>
      <url><loc>https://www.nwnatural.com/</loc></url>
      <url><loc>https://www.nwnatural.com/Products/Terra</loc></url>
      <url><loc>https://www.nwnatural.com/about-us</loc></url>
      <url><loc>https://www.nwnatural.com/Test-Drive</loc></url>
    </urlset>`;

    const filtered = filterLegacyStarterSitemapEntries(xml);

    expect(filtered).toContain('https://www.nwnatural.com/');
    expect(filtered).toContain('https://www.nwnatural.com/about-us');
