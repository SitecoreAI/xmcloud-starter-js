...
jest.mock('@/components/text-banner/TextBannerNwnStory.dev', () => ({
  TextBannerNwnStory: ({
    fields,
  }: {
    fields: { heading: { value: string } };
  }) => <div data-testid="text-banner-nwn-story">{fields?.heading?.value}</div>,
}));
...
describe('TextBanner', () => {
  it('renders Default variant with isPageEditing prop', () => {
    ...
    expect(screen.getByTestId('text-banner-nwn-story')).toBeInTheDocument();
    expect(screen.getByTestId('text-banner-nwn-story')).toHaveTextContent(
